...
include dirname(__FILE__).'/Push/Request/V20160801/PushRequest.php';
use \Push\Request\V20160801 as Push;
class Push_info{
    //消息类型 MESSAGE NOTICE

    public function push($MESSAGE_NOTICE,$ALL_DEVICE_ACCOUNT_TAG,$device,$activity,$key_value,$title,$body){
        // "{\"k1\":\"android\",\"k2\":\"v2\"}"
        // 设置你自己的AccessKeyId/AccessSecret/AppKey
        $page='com.lucky.sweet';
        $accessKeyId = "LTAIAi8loom9Fxcy";
        $accessKeySecret = "YeqdntJMTYHkOw9LCQ1CGbXqN22Pjh";
        $appKey = "24801052";
...
// 推送目标
        ...
        $request->setTarget($ALL_DEVICE_ACCOUNT_TAG); //推送目标: DEVICE:推送给设备; ACCOUNT:推送给指定帐号,TAG:推送给自定义标签; ALL: 推送给全部
        $request->setTargetValue($device); //根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
        $request->setDeviceType("ANDROID"); //设备类型 ANDROID iOS ALL.
        $request->setPushType($MESSAGE_NOTICE); //消息类型 MESSAGE NOTICE
...
// 推送配置: iOS
        ...
        $request->setiOSExtParameters($key_value); //自定义的kv结构,开发者扩展用 针对iOS设备
// 推送配置: Android
        $request->setAndroidNotifyType("BOTH");//通知的提醒方式 "VIBRATE" : 震动 "SOUND" : 声音 "BOTH" : 声音和震动 NONE : 静音
        $request->setAndroidNotificationBarType(10);//通知栏自定义样式0-100
        ...
        $request->setAndroidActivity($page.$activity);//设定通知打开的activity，仅当AndroidOpenType="Activity"有效
        ...
        $request->setAndroidExtParameters($key_value); // 设定android类型设备通知的扩展属性
// 推送控制
